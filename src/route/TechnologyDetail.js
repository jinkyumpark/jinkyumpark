...
const TechnologyDetail = () => {
	...
	useEffect(() => {
		const technologyArray = []
			...
			.filter((tech) => tech.name.trim().toLowerCase().replaceAll(' ', '') === technologyName.trim().toLowerCase().replaceAll(' ', ''))
			...
			{technology === null || typeof technology === 'undefined' ? (
				...
				<div className='container mt-5 mb-5'>
					...
					<PortfolioUsedCard
						...
						portfolios={portfolios.filter((port) => port.technologiesUsed.includes(technology.id))}
					/>
				</div>
			)}
		</>
		...
		<Card className='mb-5'>
			<div className='row'>
				<div className='col-xs-12 col-lg-3 text-center'>
					<img src={technology.icon} alt={`${technology.name}-icon`} className='img-fluid mt-3 ps-3' />
					...
					<Card.Body>
						{technology.features.length === 0 ? (
							<div className='mt-5 ms-2'>
								<Error subErrorMessage='아직 보유기술에 대한 설명이 없어요' subStyle='' delay='0' />
							</div>
						) : (
...
const BlogCard = ({ technology }) => {
	...
	return (
		<Card className='mb-5'>
			<Card.Header>
				<h4 className='text-center'>{technology.name}에 관한 블로그 포스트 보러가기</h4>
			</Card.Header>
						...
						<Card className=''>
							<Card.Body>
								<div className='hover-effect'>
									...
									/>
									<h4 className='mt-2'>{technology.name} 관련 블로그 포스트 모두보기</h4>
								</div>
							</Card.Body>
						</Card>
					</a>
...
const PortfolioUsedCard = ({ technology, portfolios }) => {
	return (
		<Card>
			<Card.Header>
				<h4 className='text-center'>{technology.name} 사용된 포트폴리오</h4>
			</Card.Header>
			<Card.Body>
				<div className='row justify-content-center'>
					{portfolios.length === 0 ? (
						<Error subErrorMessage={`기술이 사용된 포트폴리오가 없어요`} subStyle='text-black' delay={0} />
					) : (
						portfolios.map((port) => {
							return (
								<div className='col-xs-12 col-6 col-xl-4'>
									<PortfolioCard portfolio={port} textStyle='text-center text-black' />
								</div>
							)
						})
					)}
